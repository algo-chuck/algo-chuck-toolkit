...
use schwab_api_types::marketdata_params::GetPriceHistoryParams;
...
pub fn handle_price_history_command(matches: &ArgMatches) -> Result<()> {
    ...
    let end_date = matches
        ...
        .and_then(|s| s.parse().ok());
    let need_extended_hours_data = matches.get_flag("extended-hours").then_some(true);
    ...
    let data = client.get_price_history(&params)?;
