...
pub fn generate_ca_sync(ca_manager: &CaManager) -> Result<CaInfo> {
    println!("🔐 Generating Certificate Authority...");
    ...
    // Save certificate and key to disk (sync version)
    save_ca_files_sync(ca_manager, &ca_cert_pem, &ca_key_pem)?;
...
/// Generate server certificate (sync version)
pub fn generate_server_certificate_sync(ca_manager: &CaManager) -> Result<ServerCertificate> {
    println!("📜 Generating server certificate...");
    ...
    // Save server certificate and key to disk (sync version)
    save_server_files_sync(ca_manager, &server_cert_pem, &server_key_pem)?;
...
/// Save CA certificate and private key to disk with proper permissions (sync version)
fn save_ca_files_sync(ca_manager: &CaManager, cert_pem: &str, key_pem: &str) -> Result<()> {
    // Save certificate (readable by others)
    std::fs::write(ca_manager.ca_cert_path(), cert_pem)
        .with_context(|| "Failed to write CA certificate")?;
...
/// Save server certificate and private key to disk with proper permissions (sync version)
fn save_server_files_sync(ca_manager: &CaManager, cert_pem: &str, key_pem: &str) -> Result<()> {
    // Save certificate (readable by others)
    std::fs::write(ca_manager.server_cert_path(), cert_pem)
        .with_context(|| "Failed to write server certificate")?;
