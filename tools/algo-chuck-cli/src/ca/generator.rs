...
pub fn generate_ca(ca_manager: &CaManager) -> Result<CaInfo> {
    ...
    save_ca_files(ca_manager, &ca_cert_pem, &ca_key_pem)?;
...
pub fn generate_server_certificate(ca_manager: &CaManager) -> Result<ServerCertificate> {
    ...
    save_server_files(ca_manager, &server_cert_pem, &server_key_pem)?;
...
fn save_ca_files(ca_manager: &CaManager, cert_pem: &str, key_pem: &str) -> Result<()> {
...
fn save_server_files(ca_manager: &CaManager, cert_pem: &str, key_pem: &str) -> Result<()> {
    // Save certificate (readable by others)
